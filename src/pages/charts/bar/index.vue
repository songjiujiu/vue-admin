...
<template>
  <ui-container>
    ...
    <div class="ul-itemlist">
      <ul>
        <li v-for="(item,index) in goldListData.icansee_group_roles" :key="index">
          ...
          <div style="display:flex; justify-content: end; cursor: pointer;" @click="clickZbMessage(item)">
            <div class="text-tanmu">
              <span>弹幕</span>
              <span>{{item.role_stat_data_out.total.chat}}</span>
            </div>
            <div class="text-liwu">
              <span>打赏金额</span>
              <span>{{item.role_stat_data_out.total.gift_value}}</span>
            </div>
            <div class="text-dage">
              <span>浏览主播</span>
              <span>{{item.role_stat_data_out.total.anchor}}</span>
            </div>
          </div>
        </li>
      </ul>
...
export default {
  ...
  mounted(){
     this.cdaddyListItemFn()
  },
  methods: {
    async cdaddyListItemFn(){
      const {$refs, $api, form, $message, $notify, setUserInfo, $router} = this
      try{
        const result = await $api.eldestbrother.golddaddy()
        this.goldListData = result.data
      }catch(error){
        console.log(error)
      }
    },
    clickZbMessage(row){
      this.$router.push('/charts/streainfo?roleId='+row.role_id)
    },
    linkNameFn(row){
      this.$router.push('/charts/bigfiles?roleId='+row.role_id)
    }
  }
}
</script>
